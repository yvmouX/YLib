...
public class SimpleConfigurationManager implements ConfigurationManager {
    ...
    public <T> T registerConfiguration(@NotNull Class<T> configClass) throws ConfigurationException {
        ...
        // 创建配置实例
        T instance = createConfigurationInstance(configClass, metadata);
     ...
     * 创建配置实例
     */
    private <T> T createConfigurationInstance(Class<T> configClass, ConfigurationMetadata metadata) throws ConfigurationException {
        try {
