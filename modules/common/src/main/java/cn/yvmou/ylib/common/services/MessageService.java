 ...
 * @since 1.0.0
 */
public class MessageService implements cn.yvmou.ylib.api.services.MessageService {
     ...
     * @param logger 日志服务
     */
    public MessageService(@NotNull Plugin plugin, @NotNull LoggerService logger) {
        this.plugin = plugin;
        this.logger = logger;
    }
    ...
    public void sendActionBar(@NotNull Player player, @NotNull String message) {
        player.spigot().sendMessage(net.md_5.bungee.api.ChatMessageType.ACTION_BAR, TextComponent.fromLegacyText(colorize(message)));
    }
