package cn.yvmou.ylib.folia.command;
 ...
 * @since 1.0.0
 */
public class FoliaCommandManager implements CommandManager {
     ...
     * @param plugin 插件实例
     */
    public FoliaCommandManager(@NotNull Plugin plugin) {
    ...
    public void registerCommands(@NotNull String commandName, @NotNull SubCommand... subCommands) {
        ...
        // 注册主命令
        FoliaMainCommand mainCommand = new FoliaMainCommand(plugin, commandName, subCommands);
        ...
        CommandExecutor adapter = (sender, command, label, args) -> {
            // 这里我们直接返回true，因为实际的命令处理在FoliaMainCommand中完成
            return true;
        };
     ...
     * Folia主命令实现
     */
    private static class FoliaMainCommand implements org.bukkit.command.CommandExecutor, TabCompleter {
        
        private final Plugin plugin;
        private final String commandName;
        private final SubCommand[] subCommands;
        
        public FoliaMainCommand(@NotNull Plugin plugin, @NotNull String commandName, @NotNull SubCommand[] subCommands) {
            this.plugin = plugin;
