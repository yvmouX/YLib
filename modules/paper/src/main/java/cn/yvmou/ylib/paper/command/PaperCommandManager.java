package cn.yvmou.ylib.paper.command;
...
public class PaperCommandManager implements CommandManager {
    ...
    public void registerCommands(@NotNull String commandName, @NotNull SubCommand... subCommands) {
        ...
        // 注册主命令
        PaperMainCommand mainCommand = new PaperMainCommand(plugin, commandName, subCommands);
        ...
        CommandExecutor adapter = (sender, command, label, args) -> {
            // 这里我们直接返回true，因为实际的命令处理在PaperMainCommand中完成
            return true;
        };
     ...
     * Paper主命令实现
     */
    private static class PaperMainCommand implements org.bukkit.command.CommandExecutor, TabCompleter {
        
        private final Plugin plugin;
        private final String commandName;
        private final SubCommand[] subCommands;
        
        public PaperMainCommand(@NotNull Plugin plugin, @NotNull String commandName, @NotNull SubCommand[] subCommands) {
            this.plugin = plugin;
